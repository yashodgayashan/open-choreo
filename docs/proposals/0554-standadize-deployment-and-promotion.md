...
- **Git references** (lightweight Release, slower reconciliation): Release (Comparatively smaller to embedded content)+ ComponentEnvSnapshot ~50-200 KB. Recommended for GitOps workflows.
- **Embedded content** (larger Release, faster reconciliation): Release (Comparatively larger to git references) + ComponentEnvSnapshot ~50-200 KB. For UI and CLI path this is been used for the deployments and promotions.

**Note**: Both approaches generate ComponentEnvSnapshot during reconciliation with the same size . The difference is in Release size and reconciliation performance (git fetches vs direct access).
